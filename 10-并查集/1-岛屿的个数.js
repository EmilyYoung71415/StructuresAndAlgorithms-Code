...
function numIslands1(grid){
    ...
    function getUnion(i,j,key){
        // 上 左 右 下
        const dy = [0,-1,1,0],dx=[-1,0,0,1];
        for(let k=0;k<4;k++){
            let x = i+dx[k],y=j+dy[k],newIndex = x+'-'+y;
...
function numIslands2(grid){
    ...
    for(let i=0;i<rowLen;i++){
        for(let j=0;j<colLen;j++){
            if(grid[i][j]!='1') continue;
            for(let k=0;k<4;k++){// 当前元素周围的元素都尝试是否可以合并
...
function numIslands(grid){
    ...
    let count = 0,rowLen=grid.length,colLen=grid[0].length;
    const dx = [-1,1,0,0],dy=[0,0,-1,1];
    for(let i=0;i<rowLen;i++){
        for(let j=0;j<colLen;j++){
            if(grid[i][j]=='1'){
